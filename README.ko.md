...
이미 설치했다면 같은 프로젝트에서 설치 명령을 다시 실행하면 됩니다.
전역으로 설치했다면 `-g`를 붙입니다. 0.3.0에서는 Windows 경로 비교와
설명 정리 절차를 고쳤습니다. 업데이트 후 Codex에서 새 작업을 열어주세요.
...
검사합니다. 보고한 범위를 패치해 달라는 요청에 이 절차까지 포함됩니다.
개인 스킬 설명은 고치거나 유지 이유를 남깁니다. 끝내지 못한 항목도 밝힙니다.
...
Windows에서는 `python3` 대신 `py -3` 또는 `python`을 사용합니다.

## 지우지 않는 것

공식 스킬과 외부에서 설치한 스킬의 원본은 수정하지 않습니다.
Playwright와 Superpowers도 여기에 포함됩니다. Puppeteer도 같습니다.
사용자 폴더에 설치됐다고 직접 만든 스킬은 아닙니다. 설치 기록과 출처를
먼저 확인합니다. 출처가 불명확하면 개인 제작 여부부터 확인합니다.
...
둡니다. 지침을 정리하면서 기본 모델이나 스킬의 위치를 바꾸지도 않습니다.
검사 결과와 백업은 `~/.astra-xray` 아래에만 저장합니다.
...
수정 후에는 `--baseline`으로 저장해 둔 수정 전 결과와 비교할 수 있습니다.
처음 확인한 목록에 변경분을 반영합니다. 무관한 캐시 파일을 비교 목록에
끼워 넣지 않습니다. 실제 적용 여부는 새 Codex 작업에서 다시 확인합니다.
확인된 목록 없이 파일만 보고 계산한 값에는 낮은 신뢰도의 추정치라는
표시가 붙습니다.
